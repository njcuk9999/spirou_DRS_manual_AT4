...
\newcommand{\caldrifte}{cal\_DRIFT\_E2DS\_spirou.py}
\newcommand{\calbadpix}{cal\_BADPIX\_spirou.py}
...
\ParamList{ic\_order\_plot}{20}{Order to plot on ff image plot}{\calext, \calextall, \caldrift, \caldrifte}{\calff, \hadmricdp}
% Future ic_ff_order_plot
...
\ParamList{noise}{100}{The value of the noise for drift calculation}{\caldrift, \caldrifte}{\caldrift, \caldrifte}
% future ic_drift_noise

\ParamList{seuilmax}{1.e9}{The maximum flux for good (unsaturated) pixels}{\caldrift}{\caldrift, \caldrifte}
% future ic_drift_maxflux

\ParamList{size}{12}{The size around a saturated pixel to flag as unusable}{\caldrift, \caldrifte}{\spirouRV.deltavrms2d(), \spirouRV.renorm\_cosmics2d(), \spirouRV.calcvrdrifts2d()}
...
% future: ic_drift_cut_raw

\ParamList{cut}{4.5}{Define the number of standard deviations cut at in cosmic renormalisation}{\caldrifte}{\caldrifte, \spirouRV.renorm\_cosmics2d()}
% future: ic_drift_cut_e2ds

\ParamList{nomax}{28}{Define the number of orders to use (starting from 0 to max) used to get median drift  median(order0 --> order max). Note: not used in \caldrifte}{\caldrift}{\caldrift}
% future: ic_drift_n_order_max

% Future: drift_type_raw

% Future: drift_type_e2ds

% Future: ic_drift_order_plot

% ----------------------------------------------------------
\vspace{0.5cm}
\subsection{Bad pixel mask parameters}

%----------------------------------------------------------------

\ParamList{wmed}{7}{We construct a "similar" flat by taking the running median of the flag in the x dimension over a boxcar width of wmed (suggested value of ~7 pix). This assumes that the flux level varies only by a small amount over wmed pixels and that the bad pixels are isolated enough that the median along that box will be representative of the flux they should have if they were not bad}{\calbadpix}{\calbadpix}
% future: badpix_flat_med_wid

\ParamList{illum\_cut}{0.05}{Defines the illumination cut parameter. If we only cut the pixels that fractionally deviate by more than a certain amount, we are going to have lots of bad pixels in unillumnated regions of the array. We therefore need to set a threshold below which a pixels is considered unilluminated. First of all, the flat field image is normalized by its 90th percentile. This sets the brighter orders to about 1. We then set an illumination threshold below which only the dark current will be a relevant parameter to decide that a pixel is "bad"}{\calbadpix}{\calbadpix}
%future: badpix_illum_cut

\ParamList{cut\_ratio}{0.5}{The maximum differential pixel response relative to the expected value}{\calbadpix}{\calbadpix}
% future: badpix_flat_cut_ratio

\ParamList{max\_hotpix}{100.0}{Maximum flux in ADU/s to be considered too hot to be used}{\calbadpix}{\calbadpix}
% future: badpix_max_hotpix
...
% ----------------------------------------------------------
\vspace{0.5cm}
\subsection{cal\_badpix keywords}

%----------------------------------------------------------------

\KWList{BHOT}{BHOT}{Fraction of hot pixels [\%]}{\calbadpix [*\_badpixel.fits]}{\calbadpix}

\KWList{BBFLAT}{BFLAT}{Fraction of bad pixels from flat [\%]}{\calbadpix [*\_badpixel.fits]}{\calbadpix}

\KWList{BNDARK}{NDARK}{Fraction of NaN pixels in dark [\%]}{\calbadpix [*\_badpixel.fits]}{\calbadpix}

\KWList{BNFLAT}{BNFLAT}{Fraction of NaN pixels in flat [\%]}{\calbadpix [*\_badpixel.fits]}{\calbadpix}

\KWList{BBAD}{BBAD}{Fraction of bad pixels with all criteria [\%]}{\calbadpix [*\_badpixel.fits]}{\calbadpix}
